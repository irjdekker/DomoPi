...
function hue_get_xy() {
        ...
        echo ${output} | perl -n -e'/\"xy\":(\d+),/ && print $1'
        result_hue_get_xy=`echo ${output} | perl -n -e'/\"xy\":(\d+),/ && print $1'`
}
...
# Function hue_on_xy_brightness: turns light(s) on with defined starting values (xy/bri)
# $1 = xy value 
# $2 = brightness value (0-255)
# $3..$x = light(s)
# Examples: hue_on_xy_brightness [0.5,0.5] 255 1

function hue_on_xy_brightness() {
        # process all lights
        for i in ${@:3}
        do
            hue_put "{ \"on\": true, \"xy\": $1, \"bri\": $2 }" "lights/$i/state"
        done
}
...
# Function hue_setstate_xy: set the xy value of one or more lights
# Remember: this fails, if the light is not turned on!
# $1 = xy value for color temperature
# $2..$x = light(s)
# Examples: hue_setstate_xy [0.5,0.5] 3

function hue_setstate_xy() {
        # process all lights
        for i in ${@:2}
        do
                hue_put "{ \"xy\": $1 }" "lights/$i/state"
        done
}
